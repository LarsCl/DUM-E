    ...
    def trajectory_callback(self, goal_handle):
        ...
        for point in trajectory.points:
            ...
            self.arduino.send_joint_targets_position(int(positions[0] / np.pi * 1800),
                                                     ...
                                                     int(positions[5] / np.pi * 1800))
            
            self.arduino.send_joint_targets_velocity(int(velocities[0] / (2 * np.pi) * 600),
                                                     int(velocities[3] / (2 * np.pi) * 600), 
                                                     int(velocities[4] / (2 * np.pi) * 600), 
                                                     int(velocities[5] / (2 * np.pi) * 600))
